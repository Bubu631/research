...
from common import ROOT, gaussian_weights, save_csv, save_manifest, file_hash, pair_crossings, static_shrinkage
...
def simulation_reports():
    ...
    bx.text(crit*.9,.3,f'{crit:.1f}',fontsize=8,ha='right');bx.set_xlabel('Equal nominal sample size n');bx.set_ylabel('Current-state MSE');bx.legend(frameon=False,fontsize=7)
...
def main():
    ...
    values={'RawHrmse':f'{harmonised.loc["latest","rmse"]:.4f}',
            ...
            'SelectedDecline':f'{-follow.observed_selected_change:.4f}',
    ...
    inputs=list((ROOT/'results/harmonised').glob('*.csv'))+list((ROOT/'results/vintage').glob('*.csv'))
    inputs += [ROOT/'results/simulation.csv',ROOT/'results/simulation_stress.csv']
    outputs=list(FIG.glob('*.pdf'))+list(TABLE.glob('*.tex'))
    save_manifest(ROOT/'results/report_manifest.json',{'completed':True,
        'input_sha256':{str(p.relative_to(ROOT)):file_hash(p) for p in sorted(inputs)},
        'output_sha256':{str(p.relative_to(ROOT)):file_hash(p) for p in sorted(outputs)}})
