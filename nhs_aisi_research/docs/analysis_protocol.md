...
## BRFSS design locked before full replay

2026-09-11T21:33:49.617294+00:00: The retrieved common cohort has 48 reporting areas (47 states plus DC), with 206,098 eligible records in 2023 and 214,151 in 2024. Kentucky and Pennsylvania are absent from 2023, Tennessee from 2024; territories are excluded. Primary selection takes 9 of 48 areas in one comparison stratum. This is a generic worker mental-health measurement testbed, not NHS provider or departmental validation.

Every area initially reveals 20 draws. Every additional packet comprises 20 independent draws with replacement from its finite-file empirical distribution. Compressing records into 31 mental-health-day frequencies is exactly equivalent for this single outcome. This sampling design allows a uniform iid reference, does not consume the finite file and does not correspond to 20 newly recruited unique people. Initial data are included in reported total respondent-draw counts; additional budgets exclude the fixed common initial phase.

Prior calibration uses 2023 only: a common prior mean and between-area SD from its 48 complete-file area means; within-area outcome variances from its finite distributions. Controlled recruitment changes probabilities proportional to exp(±kappa*health/previous-year-SD), with a persistent random sign per area/replication. Scenarios: uniform ordinary channel (kappa=0), kappa=.25 and .5 ordinary channels with uniform validation, weight-proportional finite-file distribution with kappa=.5, and a validation-tilt failure scenario (.25 validation versus .5 ordinary). Previous-year hypothetical plus/minus sampling laws supply the bias prior centre/SD and working sampling variances. This favourable knowledge of a simulated recruitment mechanism is explicit, not an estimated real NHS/BRFSS bias or a deployable calibration procedure. Bias-blind baselines and synthetic misspecification examine failures of that knowledge.

Primary replay: 1,000 repetitions per scenario, 48 or 144 additional ordinary-packet cost equivalents. Validation costs {2,5,10} for uniform and kappa=.5 unweighted scenarios; cost5 for others. Shared sample streams pair policies, costs and budgets within scenario. Weighted sensitivity draws records proportionally to their supplied weights and targets that weighted finite-file distribution; its sample mean is unbiased for this constructed distribution, not a design-based estimate of population prevalence. It assumes access to the full recorded sampling frame and weights. The hidden 2024 complete-file means never enter allocation policy state.
