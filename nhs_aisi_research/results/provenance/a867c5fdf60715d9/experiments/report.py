...
def main():
    ...
    for name,frame,keys in [('Gaussian',sy,['bias_sd','audit_cost','budget']),('Stress',st,['scenario']),('Replay',re,['scenario','audit_cost','budget'])]:
        compact +=[r'\small',r'\begin{longtable}{lrrrrrrrrrr}',r'\caption{'+name+r': all ten policy mean regrets. Full precision and uncertainty are in the CSV release.}\\',r'\toprule',r'Setting & U & UB & V & B & O & OB & E & M & A & T \\',r'\midrule',r'\endfirsthead',r'\toprule',r'Setting & U & UB & V & B & O & OB & E & M & A & T \\',r'\midrule',r'\endhead']
        for values,g in frame.groupby(keys,sort=False):
            if not isinstance(values,tuple):values=(values,)
