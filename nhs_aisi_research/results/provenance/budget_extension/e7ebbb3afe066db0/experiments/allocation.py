...
def choose_actions(belief, policy, groups, remaining, costs, ordinary_noise,
    ...
    if policy in {'uniform','uniform_blind','two_channel_uniform','validation_only','balanced_mix'}:
        ...
        if policy in {'uniform','uniform_blind'}:score[:,:,1]=-np.inf
        elif policy=='validation_only':score[:,:,0]=-np.inf
        elif policy=='balanced_mix':
            validation=rng.random(reps)<costs[0]/sum(costs)
            score[:,:,0]=np.where(validation[:,None],-np.inf,score[:,:,0])
            score[:,:,1]=np.where(validation[:,None],score[:,:,1],-np.inf)
            # Use the affordable ordinary channel if a randomly requested audit is too expensive.
            fallback=validation&(remaining<costs[1])
            score[fallback,:,0]=rng.random((fallback.sum(),units))
