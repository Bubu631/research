...
def load_assets():
    cache = CACHE / "mlqe_pe"
    data_url = f"https://raw.githubusercontent.com/sheffieldnlp/mlqe-pe/{DATA_REV}/data/post-editing/train/en-de-train.tar.gz"
    archive = fetch(data_url, cache / "en-de-train.tar.gz")
    ...
    with tarfile.open(archive) as f:
        columns = {ext: f.extractfile(f"en-de-train/train.{ext}").read().decode("utf-8").splitlines()
    ...
    return snapshot, columns, {"data_url": data_url, "data_revision": DATA_REV,
        ...
        "model_revision": MODEL_REV, "model_license": "CC-BY-4.0",
        "model_files_sha256": {p.name: sha(p) for p in snapshot.iterdir() if p.is_file()}}


def population(columns, tokenizer):
    en, de = MosesDetokenizer(lang="en"), MosesDetokenizer(lang="de")
    ...
    for k, (src, mt, pe) in enumerate(zip(columns["src"], columns["mt"], columns["pe"]), 1):
        ...
        eligible.append({"line": k, "raw_triple_sha256": raw_hash, "src": src, "pe": pe,
            "source_ids": sid + [tokenizer.eos_token_id], "pe_ids": pid + [tokenizer.eos_token_id],
...
def main():
    ...
    manifest.update(population=pop,protocol_sha256=sha(protocol),script_sha256=sha(__file__),
