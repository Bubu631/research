...
## Second-language extension frozen before any Chinese model forward

2026-09-11: After completion of the first English–German run, extend the same frozen mechanisms and analysis to English–Chinese. This is an explicitly subsequent scope extension requested independently during the first run, not an independent preregistered replication. Report both pairs regardless of outcome.

Use the same MLQE-PE revision, file `data/post-editing/train/en-zh-train.tar.gz`, SHA-256 `9aee7f258d9a7050a6f46851ff37c6cfe4efd074a57fd692af832f2d8c864d5c`; the author archive has 7,000 real source/MT/PE triples. Model [Helsinki-NLP/opus-mt-en-zh](https://huggingface.co/Helsinki-NLP/opus-mt-en-zh), revision `408d9bc410a388e1d9aef112a2daba955b945255`, model-card license Apache-2.0. The model card requires an initial language marker; use `>>cmn_Hans<<` (Simplified Mandarin) before the English source. Source length filtering and propensity exclude this control token. Moses detokenization uses `lang=zh` for MT and PE, whose CJK rules join adjacent Han tokens and retain Latin word boundaries. Otherwise retain all filters, deterministic line-number ranking, 512 examples, 256/256 split, alignment ties, EOS convention, three propensities, alpha grid, seeds, 64-draw sample sizes and 90 one-step adapters unchanged. Save outputs in `results/public_en_zh/` and `figures/public_en_zh/`; first-pair files remain at their original locations.

Packaging correction after the first run: resolve the release root from the experiment file's parent directory, and use `PUBLIC_POSTEDIT_CACHE` or a repository-local ignored `.cache/public_postedit` path. The actual first-run script is preserved as `results/public_executed_script_en_de.py`; the path-only packaging changes do not alter its numerical results. Each run stores the exact executed source snapshot and its digest.

## Final deterministic replay and provenance preservation

After both initial runs, perform one final replay of each unchanged numerical configuration from the release directory, exercising the portable entrypoint. The script now automatically snapshots the complete protocol before computation. Preserve each original run's manifest, exact executed source and matching protocol snapshot under its `public_first_run_provenance/` directory; the current result files and manifest then describe the final replay. Compare all numeric CSV, NPY and NPZ results against the first executions, retaining discrepancies if any. This is a reproducibility check, not a new selection of examples, propensities, alpha values, step sizes or outcomes. For a clone, run `python experiments/public_postedit.py --pair en-de`, then `--pair en-zh`, and `python experiments/public_postedit_report.py` from the repository root. Downloads use `PUBLIC_POSTEDIT_CACHE` when set, or the ignored `.cache/public_postedit` directory.
