...
def variance_optimal_survival(increment_variances,costs,budget,floor=0.005):
    ...
    A zero-variance tail can leave unused budget after useful queries saturate;
    report the returned schedule's actual cost, not the budget ceiling.
    """
    ...
    if np.max(v)==0:return np.full(len(v),budget/c.sum())
    # A common positive multiplier cannot change the minimizing schedule.
    # Normalization also prevents tiny, nonzero moments being treated as zero.
    v=v/np.max(v)
    blocks=[]
    ...
    saturated=np.where(scale>0,1.,floor)
    if saturated@c<=budget:return saturated
    lo,hi=0.,max(1.,1/np.min(scale[scale>0]))
    for _ in range(80):
        mid=(lo+hi)/2
