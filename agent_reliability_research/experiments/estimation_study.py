       ...
       for regime in ['calibrated','overconfident','shifted']:
        ...
        for budget in args.budgets:
         ...
         for i,((p,r,c,l),(path,order)) in enumerate(zip(test,paths)):
            ...
            variants['ht_then_normalize']=(metrics(split_advantages(corrected),w,target,l),budget*g,budget**g,mode)
            # Raw proxy probabilities with partial replacement: cheap practical arm.
            partial=p+mask*(r-p)
            variants['partial_then_normalize']=(metrics(split_advantages(partial),w,target,l),budget*g,budget**g,mode)
