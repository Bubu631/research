...
## Dated implementation and smoke record

2026-09-12: The first `.venv` launch was interrupted before any experiment because macOS had marked the NumPy source file `dataless` and import blocked on file hydration. The bundled runtime imported NumPy 2.3.5 normally. All actual smoke/full executions use `/Users/shengweizhang/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3`.

The 18-run, five-update smoke completed in 0.800 seconds. Initial stochastic reporting success at H=12/24/48 was .375/.172/.031 (64 episodes, one seed). This exposes low signal at H48; it is retained without changing initialization, learning rate, task selection, predictor or schedule. The smoke is not evidence that any method wins. Its four exact executed source/protocol files are preserved in `results/stateful_smoke/executed_source`, matched to the first manifest hashes.

Independent integration checks passed after smoke: all 192 generated releases obey reference-success implies cheap-success; purchased-only RR cutoff enumeration reconstructs the full 16-vector to maximum error 4.44e-16; the softmax trajectory score agrees with held-action finite differences to 1.40e-11. These checks use a separate seed and are not training or parameter-selection data. Metadata-only improvements subsequently add runtime identification, automatic source snapshots and separate cheap-check accounting; no training parameter or estimator was changed.

The parent study lead authorized the complete original six-method grid on 2026-09-12 after reviewing smoke and the integration checks. Proposed parameters above are now frozen for this run. No model-derived survival arm is included in this primary stateful experiment; any such arm requires a separately recorded extension.
